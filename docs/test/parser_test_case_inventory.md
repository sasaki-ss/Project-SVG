...
| 5 | test_normal_5 | XML宣言付き入力から `svg` ルートと子要素を抽出できることを確認する。 | 正常 | - |
| 6 | test_normal_6 | 先頭・内部・末尾のコメントを無視して要素と属性を抽出できることを確認する。 | 正常 | - |
| 7 | test_normal_7 | DOCTYPE宣言付き入力から `svg` と子要素を抽出できることを確認する。 | 正常 | - |
| 8 | test_normal_8 | テキストノードを含む入力で、テキストを無視して要素のみ抽出できることを確認する。 | 正常 | - |
| 9 | test_abnormal_9 | 閉じタグの対応が不一致の入力で抽出失敗（`nullopt`）になることを確認する。 | 異常 | - |
| 10 | test_abnormal_10 | ルート要素が `svg` でない入力で抽出失敗（`nullopt`）になることを確認する。 | 異常 | - |
| 11 | test_abnormal_11 | 属性値がクォートされていない入力で抽出失敗（`nullopt`）になることを確認する。 | 異常 | - |
| 12 | test_abnormal_12 | 属性の `=` が欠落した入力で抽出失敗（`nullopt`）になることを確認する。 | 異常 | - |
| 13 | test_abnormal_13 | 開始タグが不完全な入力で抽出失敗（`nullopt`）になることを確認する。 | 異常 | - |
